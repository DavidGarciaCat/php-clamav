...
static int clamav_load_database()
{
	...
	/* database reload */
    if (cl_statchkdir(&CLAMAV_G(dbstat)) == 1) {
		reload = 1;
        if (CLAMAV_G(dbengine)) {
             cl_engine_free(CLAMAV_G(dbengine));
        }

        CLAMAV_G(dbengine) = NULL;
        CLAMAV_G(sig_num)  = 0;
	}
    ...
    /* database loading */
    if (ret = cl_load(CLAMAV_G(dbpath), CLAMAV_G(dbengine), &CLAMAV_G(sig_num), CL_DB_STDOPT)) {
    ...
    /* database stat */
    if (reload == 0 ) memset(&CLAMAV_G(dbstat), 0, sizeof(struct cl_stat));
    if (reload == 1 ) cl_statfree(&CLAMAV_G(dbstat));
    cl_statinidir(CLAMAV_G(dbpath), &CLAMAV_G(dbstat));
...
PHP_FUNCTION(cl_scanfile)
{
    ...
    /* database loading */
    if (ret = clamav_load_database()) {
        php_error(E_WARNING, "Load database during cl_scanfile failed (%s)\n", cl_strerror(ret));
        RETURN_FALSE;
    }
